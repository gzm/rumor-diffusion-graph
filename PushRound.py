...
def graph():
    global G2,infectedNodesList, edgeList, rNodeId, rEdge,infNCount,nOfRounds
    infectedNodesList = []
...
def push(rumor):
    ...
    print "After first push, infected node list:"
    print infectedNodesList
    print "Number of infected nodes:%d\n"%(infNCount)
    
   
    while infNCount != G2.GetNodes():
        for NI in G2.Nodes():
            if NI.GetId() in infectedNodesList:
                ...
                if nval > randomNVal:
                    ...
                    print "Random neighbor value after push process: %d\n" %(postNodeVal) 
                    infectedNodesList.append(randomEdge)
                    infNCount += 1
                    printNodes()
                    #if infNCount == 90:
                        #printNodes()
...
def firstPush(rumor):
    ...
    for NI in G2.Nodes():
        if NI.GetId() == rNodeId:
            ...
            infectedNodesList.append(NI.GetId())
            infNCount += 1
    return infNCount
